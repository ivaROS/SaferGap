...
#include <safer_gap/pg_mpc_hc_local_planner_ros.h>
...
void PGMpcHcLocalPlannerROS::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros)
{
    ...
    if (!_initialized)
    {
        ...
        time_dilation_ = false;
        nh.param("controller/ni/time_dilation", time_dilation_, time_dilation_);
        if(time_dilation_)
            ni_enabled_ = false;
        simple_dilation_ = true;
        nh.param("controller/ni/simple_dilation", simple_dilation_, simple_dilation_);
        ...
        */
        ...
        dyn_params_ = _controller.getDynamicLimits();
        time_dilated_traj_pub_ = nh.advertise<pips_trajectory_msgs::trajectory_points>("time_dilated_traj", 10);
        time_dilated_path_pub_ = nh.advertise<nav_msgs::Path>("time_dilated_path", 10);

        ref_traj_pub_ = nh.advertise<nav_msgs::Path>("ref_traj", 10);
        mpc_traj_pub_ = nh.advertise<nav_msgs::Path>("mpc_traj", 10);
        timing_stats_pub_ = nh.advertise<safer_gap::TimingStats>("timing_stats", 1);
...
bool PGMpcHcLocalPlannerROS::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
    ...
    opt_stats.keyhole_fail_num = static_cast<uint64_t>(_controller.getKeyholeFailNum());
    mpc_optimal_stats_pub_.publish(opt_stats);

    safer_gap::TimingStats timing_stats;
    timing_stats.sg_time = sgap_time_;
    timing_stats.traj_time = traj_time_;
    _controller.getTiming(timing_stats);
    timing_stats.total_time = (double) time_elapsed;
    timing_stats_pub_.publish(timing_stats);
                                                     ...
                                                     geometry_msgs::TwistStamped& cmd_vel, std::string& message)
{
    ...
    std::vector<potential_gap::BezierPathProfile>* bezier_paths = new std::vector<potential_gap::BezierPathProfile>();
    auto score_set = potential_gap_.initialTrajGen(gap_set, traj_set, virtual_traj_set, bezier_paths);
    geometry_msgs::PoseArray chosen_virtual_traj_set;
    int* picked_id = new int();
    auto picked_traj = potential_gap_.pickTraj(traj_set, score_set, gap_set, virtual_traj_set, chosen_virtual_traj_set, picked_id);
    ...
    if(time_dilation_ && simple_dilation_)
    {
        if(is_new_plan)
        {
            _bezier_path = bezier_paths->at(*picked_id);
            // _bezier_path.timeDilation(v_des_);
            _bezier_path.timeDilationSeg(v_des_);
            _bezier_dilated_path = _bezier_path.toPoseArray();
            _bezier_points = _bezier_path.toBezierPoints(_robot_vel.linear.x, _robot_vel.angular.z);
        }

        final_traj = _bezier_dilated_path;
    }
    ...
    ROS_INFO_STREAM_COND(print_timing_, "SGap time: " << sgap_time_elapsed << " ms.");
    sgap_time_ = (double) sgap_time_elapsed;
    ...
    ros::WallTime traj_gen_start = ros::WallTime::now();
...

    {
            ...
            if(is_new_plan)
            {
                ...
                else
                {
                    if(time_dilation_)
                    {
                        ...
                        if(time_dilation_ && simple_dilation_)
                        {
                            timed_plan_ = convertToPipsTraj(transformed_plan, _bezier_points);
                        }
                        else if(time_dilation_ && !simple_dilation_)
                            timed_plan_ = parameterizeVW2(transformed_plan, _robot_vel.linear.x, v_des_, a_des_, dyn_params_.v_max, dyn_params_.w_max);
                        else
                        {
                            ROS_ERROR_STREAM("Wrong flags for time dilation.");
                            return mbf_msgs::ExePathResult::NO_VALID_CMD;
                        }
        ...
        ros::WallDuration traj_gen_elapsed = ros::WallTime::now() - traj_gen_start;
        float traj_gen_time_elapsed = float(traj_gen_elapsed.toNSec())/1000000;
        ROS_INFO_STREAM_COND(print_timing_, "Traj gen time: " << traj_gen_time_elapsed << " ms.");
        traj_time_ = (double) traj_gen_time_elapsed;
        ...
        {
            DM u_opt, x_seq, ref;
            success = _controller.stepHC(chosen_gap, ego_min, robot_geo, timed_plan_, _robot_pose, _robot_vel, t_diff, u_opt, x_seq, ref);
            // if(success)
            {
            ...
            if(enable_vis_)
            {
                ...
                nav_msgs::Path ref_traj = xSeqToPathMsg(ref, robot_pose.header);
                ref_traj_pub_.publish(ref_traj);
            }
        }
        else
        {
    ...
    if(!use_pose_controller_ && !success && abs(cmd_vel.twist.linear.x) <= 1e-10 && abs(cmd_vel.twist.angular.z) <= 1e-10)
        return mbf_msgs::ExePathResult::NO_VALID_CMD;
    else if(use_pose_controller_ && !success && abs(cmd_vel.twist.linear.x) <= 1e-4 && abs(cmd_vel.twist.angular.z) <= 1e-4)
        return mbf_msgs::ExePathResult::NO_VALID_CMD;
    else
        return mbf_msgs::ExePathResult::SUCCESS;
}
...
void PGMpcHcLocalPlannerROS::reset()
{
    ...
    _controller.resetKeyholeFailNum();
...
pips_trajectory_msgs::trajectory_points PGMpcHcLocalPlannerROS::parameterizeVW(std::vector<geometry_msgs::PoseStamped>& raw_path, double start_v, double desired_vel, double desired_acc, double v_max, double w_max)
{
    pips_trajectory_msgs::trajectory_points timed_traj;
    nav_msgs::Path timed_traj_path;
    // timed_traj.header.stamp = ros::Time(0);
    timed_traj.header.stamp = ros::Time::now();
    timed_traj_path.header.stamp = ros::Time::now();
    if(raw_path.size() < 2)
        return timed_traj;  // TODO: modify to bool?

    timed_traj.header.frame_id = raw_path[0].header.frame_id;
    timed_traj_path.header.frame_id = raw_path[0].header.frame_id;

    // geometry_msgs::PoseStamped prev_pose = raw_path[0];
    geometry_msgs::PoseStamped prev_pose;
    prev_pose.pose.orientation.w = 1;
    double prev_time = 0;
    ROS_INFO_STREAM("Start v: " << start_v);
    double acc_time = desired_vel <= start_v ? 0 : ((desired_vel - start_v) / desired_acc);
    double acc_dist = (start_v + desired_vel) / 2 * acc_time;
    ...
    for(size_t i = 0; i < raw_path.size(); i++)
    {
        ...
        tf2::Matrix3x3 cur_m(cur_q);
        double cur_roll, cur_pitch, cur_yaw;
        cur_m.getRPY(cur_roll, cur_pitch, cur_yaw);
        ...
        tf2::Matrix3x3 prev_m(prev_q);
        double prev_roll, prev_pitch, prev_yaw;
        prev_m.getRPY(prev_roll, prev_pitch, prev_yaw);
        double theta_diff = cur_yaw - prev_yaw;
        total_dist += dist;

        double vc = desired_vel;
        if(total_dist < acc_dist)
        {
            vc = start_v + desired_acc * prev_time;
        }

        // double delta_t = dist / vc;
        // double wd = theta_diff / delta_t;
        // double vd = vc;
        // if(abs(wd) > w_max)
        // {
        //     wd = wd > 0 ? w_max : -w_max;
        //     delta_t = theta_diff / wd;
        //     vd = dist / delta_t;
        // }
        ...
        //     vd = v_max;
        //     delta_t = dist / vd;
        //     wd = theta_diff / delta_t;
        // }

        double delta_t = 0.2;
        double x_u = cos(prev_yaw);
        double y_u = sin(prev_yaw);
        double vd;
        if(x_u <= 1e-4 && y_u > 1e-4)
            vd = y_diff / y_u / delta_t;
        else if(y_u <= 1e-4 && x_u > 1e-4)
            vd = x_diff / x_u / delta_t;
        else if(x_u <= 1e-4 && y_u <= 1e-4)
            vd = 0;
        else
        {
            double x_vc = x_diff / x_u / delta_t;
            double y_vc = y_diff / y_u / delta_t;
            vd = min(x_vc, y_vc);
        }
        vd = abs(vd);
        double wd;
        if(vd < 1e-3)
        {
            delta_t = abs(theta_diff) / w_max;
            wd = theta_diff < 0 ? -w_max : w_max;
        }
        else
        {
            delta_t = dist / vd;
        ...
        pips_trajectory_msgs::trajectory_point point;
        point.time = ros::Duration(prev_time);
        PoseSE2 prev_p(prev_pose.pose);
        point.x = prev_p.x();
        point.y = prev_p.y();
        point.theta = prev_p.theta();
        point.v = vd;
        point.w = wd;

        timed_traj.points.push_back(point);

        double curr_time = prev_time + delta_t;

        if(vd < 1e-3)
        {
            while(abs(theta_diff) > 0)
            {
                theta_diff -= (delta_t * wd);
                point.time = ros::Duration(curr_time);
                point.theta += (delta_t * wd);
                point.v = 0;
                point.w = wd;
                timed_traj.points.push_back(point);
                curr_time += delta_t;
        ...
        timed_traj_path.poses.push_back(cur_pose);
    }

    time_dilated_traj_pub_.publish(timed_traj);
    time_dilated_path_pub_.publish(timed_traj_path);

    return timed_traj;
}

pips_trajectory_msgs::trajectory_points PGMpcHcLocalPlannerROS::parameterizeVW2(std::vector<geometry_msgs::PoseStamped>& raw_path, double start_v, double desired_vel, double desired_acc, double v_max, double w_max)
{
    pips_trajectory_msgs::trajectory_points timed_traj;
    nav_msgs::Path timed_traj_path;
    // timed_traj.header.stamp = ros::Time(0);
    ...
    for(size_t i = 0; i < raw_path.size(); i++)
    {
        ...
        double new_delta_x = delta_t * vd * cos(prev_yaw);
        double new_delta_y = delta_t * vd * sin(prev_yaw);
        double new_delta_theta = delta_t * wd;
        int count = 0;
        while((abs(new_delta_x) - abs(x_diff)) > 1e-3 || (abs(new_delta_y) - abs(y_diff)) > 1e-3)
        {
            if(count > 30)
            {
                vd = 0;
            ...
            vd *= 0.5;
            // delta_t = dist / vd;
            // wd = theta_diff / delta_t; // WARNING: No upper bound of wd.

            new_delta_x = delta_t * vd * cos(prev_yaw);
            new_delta_y = delta_t * vd * sin(prev_yaw);

            ROS_INFO_STREAM(abs(x_diff) << " " << abs(new_delta_x) << " " << abs(y_diff) << " " << abs(new_delta_y));
            // new_delta_theta = delta_t * wd;
        }
    ...
    return addBetweenPoints(timed_traj);
}

pips_trajectory_msgs::trajectory_points PGMpcHcLocalPlannerROS::addBetweenPoints(pips_trajectory_msgs::trajectory_points& init_traj)
{
    if(init_traj.points.size() < 2)
        return init_traj;

    pips_trajectory_msgs::trajectory_points timed_traj;
    nav_msgs::Path timed_traj_path;

    timed_traj.header = init_traj.header;
    timed_traj_path.header = init_traj.header;

    pips_trajectory_msgs::trajectory_point prev_pose = init_traj.points[0];

    double delta_t = 0.2;

    for(int i = 1; i < init_traj.points.size(); i++)
    {
        timed_traj.points.push_back(prev_pose);
        timed_traj_path.poses.push_back(trajPoint2PoseStamped(prev_pose));

        pips_trajectory_msgs::trajectory_point cur_pose = init_traj.points[i];
        // double acc_time = (cur_pose.v - prev_pose.v) / desired_acc;
        double acc_time = ((double) (cur_pose.time - prev_pose.time).toNSec()) / 1000000000;
        double cur_acc = (cur_pose.v - prev_pose.v) / acc_time;
        int bet_pt_size = (int) ceil(acc_time / delta_t) - 1;
        ...
        double prev_v = prev_pose.v;
        double prev_w = prev_pose.w;
        double prev_x = prev_pose.x;
        double prev_y = prev_pose.y;
        double prev_theta = prev_pose.theta;

        double first_bet_theta = prev_theta + delta_t * prev_w;
        double w_bet = (cur_pose.theta - first_bet_theta) / (acc_time - delta_t);

        for(int j = 1; j <= bet_pt_size; j++)
        {
            double cur_x = prev_x + delta_t * prev_v * cos(prev_theta);
            double cur_y = prev_y + delta_t * prev_v * sin(prev_theta);
            double cur_theta = prev_theta + delta_t * prev_w;

            double cur_v = prev_v + delta_t * cur_acc;
            double cur_w = w_bet;

            pips_trajectory_msgs::trajectory_point pt;
            pt.time = prev_pose.time + ros::Duration(j * delta_t);
            pt.x = cur_x;
            pt.y = cur_y;
            pt.theta = cur_theta;
            pt.v = cur_v;
            ...
            timed_traj.points.push_back(pt);
            timed_traj_path.poses.push_back(trajPoint2PoseStamped(pt));
        }

        prev_pose = cur_pose;
    }

    timed_traj.points.push_back(init_traj.points[init_traj.points.size() - 1]);
    timed_traj_path.poses.push_back(trajPoint2PoseStamped(init_traj.points[init_traj.points.size() - 1]));

    time_dilated_traj_pub_.publish(timed_traj);
    time_dilated_path_pub_.publish(timed_traj_path);

    for(int i = 1; i < timed_traj.points.size(); i++)
    {
        if(timed_traj.points[i].time < timed_traj.points[i-1].time)
        {
            ROS_ERROR_STREAM("timed smaller: " << timed_traj.points[i].time << " " << timed_traj.points[i-1].time);
            throw;
        }
    }
...
pips_trajectory_msgs::trajectory_points PGMpcHcLocalPlannerROS::convertToPipsTraj(std::vector<geometry_msgs::PoseStamped>& raw_path, std::vector<potential_gap::BezierPoint>& bpt)
{
    pips_trajectory_msgs::trajectory_points timed_traj;
    nav_msgs::Path timed_traj_path;
    ...
    if(raw_path.size() < 2)
    {
        ROS_WARN_STREAM("Path from gap is too short with [" << raw_path.size() << "] poses.");
        return timed_traj;  // TODO: modify to bool?
    }

    if(raw_path.size() != bpt.size())
    {
        ROS_ERROR_STREAM("Path and bezier points does not have the same size: [" << raw_path.size() << "] and [" << bpt.size() << "].");
        return timed_traj;  // TODO: modify to bool?
    }

    timed_traj.header = raw_path[0].header;
    timed_traj_path.header = raw_path[0].header;

    int size = raw_path.size();

    for(int i = 0; i < size; i++)
    {
        pips_trajectory_msgs::trajectory_point pt;
        pt.time = ros::Duration(bpt[i].time);
        pt.x = raw_path[i].pose.position.x;
        pt.y = raw_path[i].pose.position.y;
        pt.theta = quat2Yaw(raw_path[i].pose.orientation);
        pt.v = bpt[i].v;
        pt.w = bpt[i].w;
    ...
    // bool small = false;
    // ros::Duration start_d = timed_traj.points[0].time;
    // for(int i = 1; i < timed_traj.points.size(); i++)
    // {
    //     ROS_INFO_STREAM(start_d);
    //     if(timed_traj.points[i].time <= start_d)
    //         small = true;
    //     start_d = timed_traj.points[i].time;
    // }
    // if(small)
    //     throw;

    timed_traj_path.poses = raw_path;
...
ros::Duration PGMpcHcLocalPlannerROS::findNearTime(const pips_trajectory_msgs::trajectory_points& traj, geometry_msgs::PoseStamped robot_pose, ros::Duration prev_tdiff)
{
    ...
    if(time_dilation_ || curr_dist_sq > 0.5)
    {
...
nav_msgs::Path PGMpcHcLocalPlannerROS::xSeqToPathMsg(const DM& x_seq, std_msgs::Header header)
{
    ...
    for(size_t i = 0; i < num; i++)
    {
        ...
        point.pose.orientation = euler2Quat(x_seq(i*x_num+2).get_elements()[0]);
