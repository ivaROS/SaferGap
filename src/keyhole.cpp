...
/*bool Keyhole::synthesize(const bool &rbf_on){
    ...
    correct_corner_points(c_safe, l1_safe, l2_safe);
    c_safe_ = c_safe;
    c_unsafe_ = c_unsafe;
    l1_safe_ = l1_safe;
...
void Keyhole::correct_corner_points(MatrixXd &c_safe, MatrixXd &l1_safe, MatrixXd &l2_safe){
...
/*********************************************************
**********************************************************/
Vector2d Keyhole::push_back(const Vector2d &c, const double &d, const Vector2d &q, const Vector2d &p, const double &gam) {
    double dx = p[0] - q[0];
    double dy = p[1] - q[1];
    double xi, yi;
    Vector2d pi;
...
/*********************************************************
**********************************************************/
void Keyhole::get_training_circle(const Vector2d &xc, const double  &r, const Vector2d &q1, const Vector2d &q2,
    MatrixXd &c_safe, MatrixXd &c_unsafe, const double &inc_angle, const double &offset) {

    MatrixXd points = sweep_circle_(q1, q2, xc, inc_angle, r);
    MatrixXd grad = points - xc_.transpose().replicate(points.rows(),1);
    grad = (grad * 2.0).rowwise().normalized() * offset;

    c_unsafe = points;
    c_safe = c_unsafe - grad;
}

/*********************************************************
**********************************************************/
void Keyhole::get_training_line(const Vector2d &c, const double &d, const Vector2d &q, const Vector2d &p,
            MatrixXd &l_safe, MatrixXd &l_unsafe, const double &dis_inc, const double &offset) {
    Vector2d pv = p-q;
    double tmpD = (pv).norm();
    int num = (int)ceil(tmpD/dis_inc) + 1;
    if (num<3) {
        num = 3;
    }
    VectorXd xl(num);
    VectorXd yl(num);
    MatrixXd g(num,2);
    VectorXd onesTmp = Eigen::VectorXd::Ones(num);

    l_safe.resize(num, 2);
    l_unsafe.resize(num, 2);

    if(abs(pv[0])<tol_){
        xl = Eigen::VectorXd::Ones(num)*(q[0]+p[0])/2;
        yl = Eigen::VectorXd::LinSpaced(num, q[1], p[1]);
    }else{
        xl = Eigen::VectorXd::LinSpaced(num, q[0], p[0]);
        yl = (-d*onesTmp-c[0]*xl)/c[1];
    }
    ...
    l_safe.col(0) = xl + g.col(0)*offset;
    l_safe.col(1) = yl + g.col(1)*offset;

}

/*********************************************************
**********************************************************/
void Keyhole::get_line_ceofficients_(const Vector2d &p0, const Vector2d &p1, const Vector2d &p_check, Vector2d &c, double &d){
    Vector2d pv = p1 - p0;
    double m, val;
...
/********************************************************* 
**********************************************************/
void Keyhole::correct_points_dis_(const Vector2d &c, const double &d, const double &h, MatrixXd &safe){

    int n = safe.rows();

    VectorXd val(n);

    VectorXd vecOnes = Eigen::VectorXd::Ones(n);

    Array<bool, Dynamic, 1> mask(n);

    val = safe*c + vecOnes*d;

    mask = val.array().abs()>h;

    MatrixXd tmp(mask.count(),2);

    tmp = filterMat_(mask, safe);

    safe.resize(mask.count(), 2);
...
/********************************************************* 
**********************************************************/
void Keyhole::correct_pints_(const Vector2d &c1, const double &d1, const Vector2d &c2, const double &d2, MatrixXd &safe1, MatrixXd &safe2){
    int n1 = safe1.rows();
    int n2 = safe2.rows();
    ...
    VectorXd vecOnes1 = Eigen::VectorXd::Ones(n1);
    VectorXd vecOnes2 = Eigen::VectorXd::Ones(n2);

    Array<bool, Dynamic, 1> mask1(n1);
    Array<bool, Dynamic, 1> mask2(n2);

    // eval
    val1 = safe1*c2 + vecOnes1*d2;
    val2 = safe2*c1 + vecOnes2*d1;

    mask1 = val1.array()>0;
    mask2 = val2.array()>0;

    MatrixXd tmp1(mask1.count(),2);
    MatrixXd tmp2(mask2.count(),2);

    tmp1 = filterMat_(mask1, safe1);
    tmp2 = filterMat_(mask2, safe2);

    safe1.resize(mask1.count(), 2);
    safe2.resize(mask2.count(), 2);
...
/********************************************************* 
**********************************************************/
MatrixXd Keyhole::filterMat_(Array<bool, Dynamic, 1> &filter, MatrixXd mat){
    int nrowf = filter.count();
    int nrow = mat.rows();
    int ncol = mat.cols();
    int k = 0;

    MatrixXd matf(nrowf, ncol);

    for(int i=0; i<nrow; i++){
        if(filter(i)){
            matf.row(k) = mat.row(i);
            k++;
        }
    }
...
/********************************************************* 
**********************************************************/ 
bool Keyhole::is_in_(const Vector2d &v, const Vector2d &p, const Vector2d & q){
  ...
  // if(((x-x_low)>tol_) && ((x_high-x)>tol_)){
  //     flag = true;
  //   }else{
  //     flag = false;
  //   }
  
    // cout << "x=" << x << "; y="<< y << "x1=" << x1 << "; x2=" << x2 << "; y1=" << y1 << "; y2=" << y2 << "; flag=" << flag << endl;
...
bool Keyhole::synthesize(const bool &rbf_on){
    ...
    Vector2d c1, c2, c3, c4, c5;
    Vector2d q1_s, q2_s;
    Vector2d p_int, p4, p5;
    double d1, d2, d3, d4, d5;
    double x_int, y_int, tmp_int, tmpD;
    int num_s, num_us, num_safe_syn, num_unsafe_syn;
    double dis_inc = ang_inc_*r_;
    double gam_push = 0.2;
    bool with_l5 = false;
    MatrixXd safe_points;
    MatrixXd unsafe_points;
    double dis_l4 = 0.1;
    // cout << dis_inc << endl;


    MatrixXd c_safe, c_unsafe, l1_safe, l1_unsafe, l2_safe, l2_unsafe, l5_safe, l5_unsafe;
    // compute intersection
        // compute coefficient
    if(mode_triangle_on_){
        q1_s = get_intersection_(xc_, r_, p0_, p1_);
        q2_s = get_intersection_(xc_, r_, p0_, p2_);
    }else{
        q1_s = q1_;
        q2_s = q2_;
    }
    get_line_ceofficients_(q1_s, p1_, q2_, c1, d1);
    get_line_ceofficients_(q2_s, p2_, q1_, c2, d2);
    get_line_ceofficients_(q1_s, q2_s, p1_, c3, d3);
    ...
    if(abs(tmp_int)>tol_) {
        x_int =  (c2(1)*d1-c1(1)*d2)/tmp_int;
        y_int = -(c2(0)*d1-c1(0)*d2)/tmp_int;
        p_int << x_int, y_int;
        pt_int_.row(0) = p_int;
    }

    if( (abs(tmp_int)<=tol_) || mode_triangle_on_ ) {
        // get training data for line 1 and 2
        get_training_line(c1, d1, q1_s, p1_, l1_safe, l1_unsafe, dis_inc, offset_);
        get_training_line(c2, d2, q2_s, p2_, l2_safe, l2_unsafe, dis_inc, offset_);
        get_training_circle(xc_, r_, q1_s, q2_s, c_safe, c_unsafe, ang_inc_, offset_);
        // correct points
        correct_pints_(c1, d1, c2, d2, l1_safe, l2_safe);
        // cout << "parallel" << endl;
    }else if( is_in_(pt_int_.row(0),q1_s,p1_) && is_in_(pt_int_.row(0),q2_s,p2_) ) {
        // push back p1 and p2
        p1_ = p_int;
        p2_ = p_int;
        p1_ = push_back(c1, d1, q1_s, p1_, gam_push);
        p2_ = push_back(c2, d2, q2_s, p2_, gam_push);
        // get training data for line 1 and 2 and circle
        get_training_line(c1, d1, q1_s, p1_, l1_safe, l1_unsafe, dis_inc, offset_);
        get_training_line(c2, d2, q2_s, p2_, l2_safe, l2_unsafe, dis_inc, offset_);
        get_training_circle(xc_, r_, q1_s, q2_s, c_safe, c_unsafe, ang_inc_, offset_);
        // correct points
        ...
        // get p4 and line 4 coefficients
        tmpD = c1.dot(p2_)+d1;
        p4 = p2_ - c1*tmpD;
        get_line_ceofficients_(p2_, p4, p1_, c4, d4);
        // get p5 and line 5 coefficients
        p5 = p1_ + c1*tmpD;
        get_line_ceofficients_(p2_, p5, p1_, c5, d5);
        // get training data for line 1, 2, and 5 and circle
        get_training_line(c1, d1, q1_s, p1_, l1_safe, l1_unsafe, dis_inc, offset_);
        get_training_line(c2, d2, q2_s, p2_, l2_safe, l2_unsafe, dis_inc, offset_);
        get_training_line(c5, d5, p2_, p5, l5_safe, l5_unsafe, dis_inc, offset_);
        get_training_circle(xc_, r_, q1_s, q2_s, c_safe, c_unsafe, ang_inc_, offset_);
        // correct points
        correct_pints_(c1, d1, c2, d2, l1_safe, l2_safe);
        correct_pints_(c1, d1, c5, d5, l1_safe, l5_safe);
        correct_points_dis_(c4, d4, dis_l4, l1_safe);
        correct_points_dis_(c4, d4, dis_l4, l2_safe);
        correct_points_dis_(c4, d4, dis_l4, l5_safe);
        // cout << "L2 extends to L1" << endl;
        with_l5 = true;
        extend_gap_ = true;
    }else if( (!is_in_(pt_int_.row(0),q1_s,p1_)) && is_in_(pt_int_.row(0),q2_s,p2_) ) {
        // get p4 and line 4 coefficients
        tmpD = c2.dot(p1_)+d2;
        p4 = p1_ - c2*tmpD;
        get_line_ceofficients_(p1_, p4, p2_, c4, d4);
        // get p5 and line 5 coefficients
        p5 = p2_ + c2*tmpD;
        get_line_ceofficients_(p1_, p5, p2_, c5, d5);
        // get training data for line 1, 2, and 5 and circle
        get_training_line(c1, d1, q1_s, p1_, l1_safe, l1_unsafe, dis_inc, offset_);
        get_training_line(c2, d2, q2_s, p2_, l2_safe, l2_unsafe, dis_inc, offset_);
        get_training_line(c5, d5, p1_, p5, l5_safe, l5_unsafe, dis_inc, offset_);
        get_training_circle(xc_, r_, q1_s, q2_s, c_safe, c_unsafe, ang_inc_, offset_);
        // correct points
        correct_pints_(c1, d1, c2, d2, l1_safe, l2_safe);
        correct_pints_(c2, d2, c5, d5, l2_safe, l5_safe);
        correct_points_dis_(c4, d4, dis_l4, l2_safe);
        correct_points_dis_(c4, d4, dis_l4, l1_safe);
        correct_points_dis_(c4, d4, dis_l4, l5_safe);
        // cout << "L1 extends to L2" << endl;
        with_l5 = true;
        extend_gap_ = true;
    }else {
        // get training data for line 1 and 2 and circle
    ...
    // get line 3 coefficients
    get_line_ceofficients_(q1_s, q2_s, p1_, c3, d3);

    // collect training data
    if(with_l5){
        num_s = c_safe.rows() + l1_safe.rows() + l2_safe.rows() + l5_safe.rows();
        num_us = c_unsafe.rows() + l1_unsafe.rows() + l2_unsafe.rows() + l5_unsafe.rows();
    }else{
        num_s = c_safe.rows() + l1_safe.rows() + l2_safe.rows();
        num_us = c_unsafe.rows() + l1_unsafe.rows() + l2_unsafe.rows();
    }

    safe_points.resize(num_s, 2);
    unsafe_points.resize(num_us, 2);

    if(with_l5){
        safe_points << c_safe, l1_safe, l2_safe, l5_safe;
        unsafe_points << c_unsafe, l1_unsafe, l2_unsafe, l5_unsafe;
    }else{
        safe_points << c_safe, l1_safe, l2_safe;
        unsafe_points << c_unsafe, l1_unsafe, l2_unsafe;
    }

    safe_points_syn_ = safe_points;
    unsafe_points_syn_ = unsafe_points;
    num_safe_syn = num_s;
    num_unsafe_syn = num_us;

    // set private c's and ds
    c1_ = c1;
    c2_ = c2;
    c3_ = c3;
    c4_ = c4;
    ...
    // safe points
    for (int i = 0; i < num_safe_syn; i++){
        tmpx << safe_points_syn_(i, 0), safe_points_syn_(i, 1);
        model(tmpx, rbf_on_, g, lims, cost);
        c[i] = solver->MakeRowConstraint(1.0, inf);
        for(int j=0; j<a_model; j++){
            c[i]->SetCoefficient(a[j], g(j));
    ...
    // unsafe points
    for (int i = 0; i < num_unsafe_syn; i++){
        tmpx << unsafe_points_syn_(i,0), unsafe_points_syn_(i,1);
        model(tmpx, rbf_on_, g, lims, cost);
        c[i] = solver->MakeRowConstraint(-inf, -1.0);
        for(int j=0; j<a_model; j++){
            c[i]->SetCoefficient(a[j], g(j));
  ...
  if (result_status != MPSolver::OPTIMAL) {
    // LOG(FATAL) << "The problem does not have an optimal solution!";
        // cout << "Warning: no optimal slution!!" << endl;
        ROS_WARN_STREAM("The problem does not have an optimal solution!");
        synthesis_done_ = false;
        return false;
  }
